...
async function main() {
    ...
    const calldata = iface.encodeFunctionData("setPendingAdmin", [MS])
    let txn = await dao.connect(proposer)["propose(address[],uint256[],string[],bytes[],string)"](
        [timelock.address], [0], [""], [calldata], "send funds")

    console.log(`Proposing to set pending admin to multisig.`)
    ...
    if (MS != await timelock.pendingAdmin()) {
        throw new Error("incorrect pending admin")
    }
}
